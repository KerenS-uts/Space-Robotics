...
class ParticleFilter(Node):
    ...
    def initialise_particles(self):
        ...
        for i in range(self.num_particles_):
            # Randomly sample x within the map's x boundaries
            x = random_uniform(self.map_x_min_, self.map_x_max_)
            # Randomly sample y within the map's y boundaries
            y = random_uniform(self.map_y_min_, self.map_y_max_)
            # Randomly sample orientation theta between 0 and 2*pi
            theta = random_uniform(0.0, 2 * math.pi)
            # Assign equal initial weight to each particle
            weight = 1.0 / self.num_particles_  # Equal initial weights
            # Create a new Particle object with the sampled values
    ...
    def clicked_point_callback(self, clicked_point_msg):
        ...
        # Generate particles around the clicked point
        for i in range(self.num_particles_):
            # Sample x from a Gaussian centered at the clicked x, with specified std deviation
            x = np.random.normal(point_x, self.clicked_point_std_dev_)
            # Sample y from a Gaussian centered at the clicked y, with specified std deviation
            y = np.random.normal(point_y, self.clicked_point_std_dev_)
            # Sample orientation theta uniformly between 0 and 2*pi
            theta = random_uniform(0.0, 2 * math.pi)
            # Assign equal initial weight to each particle
            weight = 1.0 / self.num_particles_
            # Create the particle and add to the list
            particle = Particle(x, y, theta, weight)
            self.particles_.append(particle)
...
# Calculate the sum of all weights
    ...
    def estimate_pose(self):
        ...
        # Compute weighted average for position and orientation
        #Formula: x̄ = Σ(x_i * w_i), ȳ = Σ(y_i * w_i)
        estimated_pose_x = sum(p.x * p.weight for p in self.particles_)
        estimated_pose_y = sum(p.y * p.weight for p in self.particles_)

        # Compute weighted average for theta (orientation)
        # We cannot simply average angles because of wrap-around (0 and 2π issue)
        # Convert angles to Cartesian components and weight them
        sum_sin = sum(math.sin(p.theta) * p.weight for p in self.particles_)
        sum_cos = sum(math.cos(p.theta) * p.weight for p in self.particles_)
        estimated_pose_theta = math.atan2(sum_sin, sum_cos)
        # Wrap angle to ensure it is within [0, 2π]
    ...
    def odom_callback(self, odom_msg):
        ...
        for p in self.particles_:
            ...
            p.x += noisy_distance * math.cos(p.theta)
            p.y += noisy_distance * math.sin(p.theta)

            # Update orientation with wrapped angle
            p.theta = wrap_angle(p.theta + noisy_rotation)

            #After motion update, normalise weights 
            self.normalise_weights()
    ...
    def terrain_callback(self, terrain_msg):
        ...
        if self.use_terrain_:
            ...
            #likelihood = math.exp(-(error ** 2) / (2 * sigma ** 2)) / (math.sqrt(2 * math.pi) * sigma)
                ...
                #likelihood = 1.0
                #Observed terrain from the sensor message 
                observed_class = terrain_msg.data
                #Actual terrain class at this particle's position
                actual_class = self.visual_terrain_map_.get_ground_truth(p.x, p.y)
                #Compute likelihood using confusion matrix 
                likelihood = self.visual_terrain_map_.confusion_matrix[actual_class, observed_class]
        ...
        if self.use_laser_:
            ...
            for p in self.particles_:
                ...
                for i in range(0, len(scan_msg.ranges), step): 
                   ...
                   # Formula: P(z | x) ∝ exp(-( (scan_range - particle_range)^2 ) / (2 * σ_z^2)) / sqrt(2π * σ_z^2)

                sigma = self.sensing_noise_stddev_

                # If either scan_range or particle_range is invalid, skip this ray
                if scan_range < scan_msg.range_min or scan_range > scan_msg.range_max or particle_range < 0:
                    continue

                # Compute the difference between observed and expected range
                diff = particle_range - scan_range

                # Compute Gaussian likelihood for this ray
                ray_likelihood = (1.0 / (math.sqrt(2.0 * math.pi) * sigma)) * math.exp(-(diff * diff) / (2.0 * sigma * sigma))

                # Multiply this ray's likelihood into the overall likelihood for the particle
    ...
    def compass_fusion(self):
        ...
        ####################
         ...
         # Check if we have a valid compass measurement
        if not self.compass_valid_:
            return

        # Loop through each particle and update its weight based on compass reading
        for p in self.particles_:
            # Calculate the orientation difference between particle and compass measurement
            angle_diff = wrap_angle(p.theta - self.compass_)

            # Compute Gaussian likelihood for this difference
            # Formula: exp( - (error^2) / (2 * sigma^2) )
            likelihood = math.exp(- (angle_diff ** 2) / (2 * (self.magnetometer_noise_stddev_ ** 2)))

            # Update the particle's weight by multiplying with the likelihood
            p.weight *= likelihood

        # Normalise weights after applying compass fusion
        self.normalise_weights()
